#!/usr/bin/python3
# to do :
# make a proper ui , gotta rethink how the selection is done
import os
import numpy as np
...
class FindSubset:
    ...
    def search_callback(self):
        ...
        try:  # spool the images into a list
        ...
        self.queue = Queue()  # queue of image pairs to process main x possible sub
        for i in images:
            for j in images:
                if i != j:
                    self.queue.put((i, j))
    ...
    def process_queue(self):
        ...
        if (temp_img.size[0] < orig_img.size[0]) and (temp_img.size[1] < orig_img.size[1]):
            ...
            if ratio != 1:  # if images were resized, get originals and calculate corresponding match_loc
                match_loc = (int(match_loc[0] / ratio),
                             int(match_loc[1] / ratio))
                orig_img = Image.open(os.path.join(self.path, pair[0]))
                temp_img = Image.open(os.path.join(self.path, pair[1]))
            ...
            sum_of_squares = sum(value * ((idx % 256) ** 2)
                                 for idx, value in enumerate(h_diff))
            rms = sqrt(sum_of_squares /
                       float(temp_img.size[0] * temp_img.size[1]))

            if RMS_THRESHOLD > rms:  # add matches to table
                self.results_table.grid(
                    row=3, column=0, columnspan=2, padx=5, pady=5)
                self.results_table.insert('', 'end', str(
                    self.progress_var.get()), text=pair[0])
                self.results_table.set(
                    str(self.progress_var.get()), 'subset', pair[1])
                self.results_table.config(height=len(
                    self.results_table.get_children('')))

        self.progressbar.step()
        self.status_var.set(
            'Analyzed {} vs {} - {} pairs remaining...'.format(pair[0], pair[1], self.queue.qsize()))
